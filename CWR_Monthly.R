...
getwd()
setwd("/Users/conorrickard/Documents/R_prog_files/_Git_Projects/ForLoop_Aggregate_MonthlyTotalsFONR")

# For Loop to read and create a new copy of each file with correct POSIXct formated time:

# Place the files you want read into the working directory folder
# Set some parameters 
extension <- "csv"  # Type of files
fileNames <- Sys.glob(paste("*.", extension, sep = "")) # List of file names in the working directory
fileNumbers <- seq(fileNames); fileNumbers # A counter to identify each file's position in the for loop sequence
